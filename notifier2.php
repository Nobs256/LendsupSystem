...
while($selected= mysqli_fetch_array($select)){  
    ...
    // NPL Calculation: Loans older than 1 year from report date
    $npl_query = mysqli_query($conn,"
        SELECT COUNT(DISTINCT cwl.clientsid) as npl_count 
        FROM clients_with_loan cwl
        JOIN loans l ON cwl.clientsid = l.cliente_id
        JOIN clients c ON c.client_id = cwl.clientsid
        WHERE c.b_location='$location' 
        AND l.userse_id='$user_id' 
        AND l.bossese_id='$boss_id'
        AND DATEDIFF('$d', l.b_date) >= 365
    ");
    ...
    $npl_with_payment_query = mysqli_query($conn,"
        ...
        FROM loan_pay lp
        JOIN loans l ON lp.loanNo = l.loan_id
    ...
    // Unpaid calculation: subtract advance, paid clients, and NPL clients who paid today
    $unpaid = $total_no_clients - $clients_paid_field - $clients_advance - $npl_with_payment;
...
// Global NPL Calculation: Total NPL clients across all locations
$global_npl_query = mysqli_query($conn,"
    SELECT COUNT(DISTINCT cwl.clientsid) as npl_count 
    FROM clients_with_loan cwl
    JOIN loans l ON cwl.clientsid = l.cliente_id
    WHERE l.userse_id='$user_id' 
    AND l.bossese_id='$boss_id'
    AND DATEDIFF('$d', l.b_date) >= 365
");
...
$global_unpaid = $loan_clients - $no_clients_paid - $total_clients_advance - $global_npl_with_payment;
...
} // Closing for if(isset($_POST['send']))

$html = ob_get_contents(); 
ob_end_clean();
 
//mPDF generation script... (assuming mPDF setup here as usual)
// require_once __DIR__ . '/vendor/autoload.php';
// $mpdf = new \Mpdf\Mpdf();
// $mpdf->WriteHTML($html);
// $mpdf->Output();

echo $html; // Echo the buffered output if not routing to mPDF right away
?>