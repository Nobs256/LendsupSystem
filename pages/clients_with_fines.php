...
$select = mysqli_query($conn,"SELECT * FROM loan_fines where  user_id='$user_id' and boss_id='$boss_id' and fine_type='Fined'");
...
?>
<div id="main_heading"> 
<table><tr><td><b>All Clients With Fines</b> (<?php echo $total_amos;?>) </td><td><font color="#EAEAEA">----------------------------------</font></td>
<td>
<form method="post">    
...
and boss_id='$boss_id' and fine_type='Fined'");
while($selected= mysqli_fetch_array($select)){  
$amount= $selected["amount"]; 
$total_amount+=$amount;
...
<tbody>";
$amounts=0;
$search_query= mysqli_query($conn,"SELECT client_id, ucase(firstname) as firstname, ucase(lastname) as lastname, pay_date, amount, fine_type, ts_id
FROM clients, loan_fines where users_id='$user_id' and bosses_id='$boss_id' and client_id=clients_id and  MONTH(pay_date)='$month'  and fine_type='Fined' order by pay_date");
 while($returned_result = mysqli_fetch_assoc($search_query)){
$j++;

$fined_id=$returned_result["ts_id"];
...
<td>
<a style='border: 1px solid #006F37; border-radius:1px; font-size:10px; color:red; height:20px' 
class='button is-default' href='clients_with_fines.php?client_id=$client_id&fined_id=$fined_id'>&nbsp;
Delete &nbsp;</a></td>";

}

if(isset($_GET['client_id'])){
$fined_id = $_GET['fined_id'];
...
$query ="DELETE from loan_fines where clients_id='$client_id' and boss_id='$boss_id' and ts_id='$fined_id'
and fine_type='Fined'";
$execute = mysqli_query($conn, $query);

//clients with loans table
$query ="UPDATE clients_with_loan set debt='$new_debt' where clientsid='$client_id' and bosseseid='$boss_id'";
$execute = mysqli_query($conn, $query);

$query ="UPDATE loan_pay set balance='$new_debt' where clients_id='$client_id' and userse_id='$user_id' and bossese_id='$boss_id' and p_date='$last_date'";
$execute = mysqli_query($conn, $query);


echo"<div style='background-color:#006F37; border-radius:5px; color:white; 
height:40px; margin-left:25px; padding:7px; width: 600px'>Fine is Succesfully Deleted !
