...
xmlhttp.open("GET","admin_connector.php?update_pay_loan=1&boss_id="+boss_id+"&client_id="+client_id+"&change_date="+change_date+"&p_date="+p_date+"&loan_no="+loan_no+"&pre_balance="+pre_balance+"&amo="+amo,true);
xmlhttp.send();
}
}
...
$j=0;
$month=date('m');
$search_query= mysqli_query($conn,"SELECT * FROM loan_pay  where  clients_id='$client_id' and bossese_id='$boss_id'  order by p_date  DESC Limit 1");
...
<td> $amount</td> 
<td> $balance </td> 
<td> <a href='error_pay_loan.php?client_id=$clients_id&Payment=$loan_id'>Update</td>
<td> <a href='error_pay_loan.php?client_id=$clients_id&delete=$loan_id'><font color=red>Delete</font></td>
";
echo "</tr>";
}
echo "</tbody></table>";
...
<a href='search_client_errors.php' style='color:white; margin-left:100px;''>Back</a></div>";
}
...
$query ="DELETE from field_payment where client_id='$client_id' and boss_id='$boss_id' and pay_date='$p_date' and mom=0";
$execute = mysqli_query($conn, $query);
...
<a href='search_client_errors.php' style='color:white; margin-left:100px;''>X</a></div>";
