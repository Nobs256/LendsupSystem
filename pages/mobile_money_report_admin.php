...
<td width="40%"><b>Monthly Mobile Money Report</b></td>
<td width="60%">
<form method="post"> 
...
//Total money sent  in this month
$select = mysqli_query($conn,"SELECT * FROM mobile where  bossmo_id='$boss_id' and MONTH(mom_date)='$month' and YEAR(mom_date)='$year' and usermo_id='$user_id'");
while($selected= mysqli_fetch_array($select)){  
$amount= $selected["amount_mo"]; 
$total_amount+=$amount;
...
echo "<b>Money Sent on MOM in <i>$mon - $year</i> is <u>$total_amounts</u> from <i>$branch</i></b>";
echo "<br><br>";
$j=0;
$search_query= mysqli_query($conn,"SELECT client_id, ucase(firstname) as firstname, ucase(lastname) as lastname, amount_mo, mom_date 
FROM clients, mobile  where users_id='$user_id' and bosses_id='$boss_id' and client_id=clientmo_id and  MONTH(mom_date)='$month' and YEAR(mom_date)='$year' order by mom_date");
...
$amount_sent  =number_format($returned_result["amount_mo"]);
$date = $returned_result["mom_date"];
$d=date("d-m-Y", strtotime($date));
 
//===paid amount 
$result = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM loan_pay where clients_id='$client_id'
and userse_id='$user_id' and bossese_id='$boss_id' and p_date='$date' and mom=1 "));     
$amount_paid = number_format($result['amount_paid']);
  
echo "<tr>
<td><font size=3> $j  </font></td>
<td><font size=3> $d  </font></td>
<td><font size=3> $firstname  </font></td>
<td><font size=3> $amount_sent</font></td>
