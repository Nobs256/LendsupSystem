...
xmlhttp.open("GET","user_connector.php?mom_balance_withdraw=1&boss_id="+boss_id+"&user_id="+user_id+"&b_date="+b_date+"&amo="+amo,true);
xmlhttp.send();
}
}
...
<tbody>";
$search_query= mysqli_query($conn,"SELECT * FROM withdraws  where user_id='$user_id'
 and boss_id='$boss_id' and MONTH(with_date)='$curr_month' and withdraws='Charges' order by with_date");
while($returned_result = mysqli_fetch_assoc($search_query)){
$j++;

$amount =number_format($returned_result["amount"]);
$date = $returned_result["with_date"]; 
